...
class LoadNifti extends React.Component{
    ...
    componentDidMount(){
        ...
        let files = t2.map(function(v) {
            return 'http://localhost/get_files/get_file.cgi?folder=' + v;
        });
        ...
        loader.load(files)
            .then(function() {
                ...
                stackFolder1.add(
                    r1.stackHelper.slice, 'windowWidth', 1, stack.minMax[1] - stack.minMax[0])
                    .step(1).listen();
                stackFolder1.add(
                    r1.stackHelper.slice, 'windowCenter', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                stackFolder1.add(
                    r1.stackHelper.slice, 'lowerThreshold', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                stackFolder1.add(
                    r1.stackHelper.slice, 'upperThreshold', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                ...
                stackFolder2.add(
                    r2.stackHelper.slice, 'windowWidth', 1, stack.minMax[1] - stack.minMax[0])
                    .step(1).listen();
                stackFolder2.add(
                    r2.stackHelper.slice, 'windowCenter', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                stackFolder2.add(
                    r2.stackHelper.slice, 'lowerThreshold', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                stackFolder2.add(
                    r2.stackHelper.slice, 'upperThreshold', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                ...
                stackFolder3.add(
                    r3.stackHelper.slice, 'windowWidth', 1, stack.minMax[1] - stack.minMax[0])
                    .step(1).listen();
                stackFolder3.add(
                    r3.stackHelper.slice, 'windowCenter', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                stackFolder3.add(
                    r3.stackHelper.slice, 'lowerThreshold', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                stackFolder3.add(
                    r3.stackHelper.slice, 'upperThreshold', stack.minMax[0], stack.minMax[1])
                    .step(1).listen();
                ...
                function onDoubleClick(event) {
                    ...
                    if (intersects.length > 0) {
                        ...
                        // onRedChanged();
                        // onYellowChanged();
                        r1.controls.addEventListener('mouseup', function() {
                            console.log()
                        ...
                        r1.controls.addEventListener('mousemove', function(evt) {
                            console.log(evt)
                            // if something hovered, exit
                            let cursor = 'default';
                            for (let widget of widgets) {
                            ...
                            r1.controls.style.cursor = cursor;
                        });

                        r1.controls.addEventListener('mousedown', function(evt) {
                            console.log(evt)
                            // if something hovered, exit
                            for (let widget of widgets) {
                                if (widget.hovered) {
                            ...
                            r1.controls.style.cursor = 'default';
                        });

                        let widget = new WidgetsFreehand(r1.stackHelper.slice.mesh, r1.controls, stack);
                        // //
                        widget.worldPosition = intersects[0].point;
                        console.log(widget.worldPosition)
                        widgets.push(widget);
                        scene.add(widget);
                    }
                }
                ...
                // event listeners
                r1.domElement.addEventListener('click', onDoubleClick);
                r2.domElement.addEventListener('click', onDoubleClick);
                r3.domElement.addEventListener('click', onDoubleClick);
